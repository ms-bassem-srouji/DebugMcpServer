...
public class StepInToolTests
{
    ...
    private static StepInTool CreateTool(DapSessionRegistry registry)
    {
        ...
        return new StepInTool(registry, options, NullLogger<StepInTool>.Instance);
    }
    ...
    private static FakeSession CreatePausedSessionWithStoppedEvent()
    {
        ...
        session.SetupRequest("stepIn", _ => new JsonObject());
    ...
    [TestMethod]
    public async Task Sends_StepIn_Command()
    {
        ...
        session.SentRequests[0].Command.Should().Be("stepIn");
    }
