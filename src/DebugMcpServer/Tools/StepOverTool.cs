...
internal sealed class StepOverTool : ExecutionToolBase, IMcpTool
{
    private readonly DapSessionRegistry _registry;
    private readonly DebugOptions _options;
    private readonly ILogger<StepOverTool> _logger;

    public string Name => "step_over";
    public string Description => "Step over the current line (execute it without entering any function calls). Process must be paused.";
    ...
    public StepOverTool(DapSessionRegistry registry, IOptions<DebugOptions> options, ILogger<StepOverTool> logger)
    {
    ...
    public async Task<JsonNode> ExecuteAsync(JsonNode? id, JsonNode? arguments, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await session.SendRequestAsync("next", new { threadId = session.ActiveThreadId ?? 1 }, cancellationToken);
            session.TransitionToRunning();
            return await WaitForStoppedResultAsync(session, id, waitSeconds, _logger, cancellationToken);
        }
        catch (DapSessionException ex) { return CreateTextResult(id, DapErrorHelper.Humanize("next", ex.Message), isError: true); }
